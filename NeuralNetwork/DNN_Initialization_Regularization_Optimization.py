...
def load_dataset(trainDataDir, testDataDir):
    ...
    # 将标签数据集转换为行向量
    train_set_y = train_set_y_orig.reshape((1, train_set_y_orig.shape[0]))  # 原始训练集的标签集设为（1*1113）
    test_set_y = test_set_y_orig.reshape((1, test_set_y_orig.shape[0]))     # 原始测试集的标签集设为（1*100）
...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    shuffled_X = X[:, permutation]                # 随机打乱X原有顺序
    shuffled_Y = Y[:, permutation].reshape((1, m))

    num_complete_minibatches = math.floor(m / mini_batch_size)  # mini_batch number
...
# 训练网络参数
def train_model(X, Y, layer_dims, lambd, keep_prob, mini_batch_size, num_iterations, learning_rate, optimizer, beta,
                beta1, beta2, epsilon, print_cost=True):
    costs = []
    seed = 10
    ...
    for i in range(num_iterations):
        ...
        for minibatch in minibatches:
            ...
            # 向前传播
            if keep_prob==1:
                AL, caches = train_model_forward(minibatch_X, parameters)
            elif keep_prob<1:
                AL, caches = train_model_forward_with_dropout(minibatch_X, parameters, keep_prob)

            # 损失函数
            if lambd==0:
                cost = compute_cost(AL, minibatch_Y)
            else:
                cost = compute_cost_with_L2Regularization(AL, minibatch_Y, parameters, lambd)

            # 向后传播
            if lambd == 0 and keep_prob == 1:
                grads = train_model_backward(AL, minibatch_Y, caches)
            elif keep_prob < 1:
                grads = train_model_backward_with_dropout(AL, minibatch_Y, caches, keep_prob)    # DropOut正则化
            elif lambd != 0:
                grads = train_model_backward_with_regularization(AL, minibatch_Y, caches, lambd)  # L2正则化
...
if __name__ == '__main__':
    ...
    train_set_x, train_set_y, test_set_x, test_set_y, classes = load_dataset(trainDataDir, testDataDir)

    # 定义网络层数及神经元个数
    n_x = train_set_x.shape[0]
    layer_dims = [n_x, 5, 5, 3, 1]

    # 训练网络
    # optimizer= gd、momentum、adam
    parameters = train_model(train_set_x, train_set_y, layer_dims, lambd=0, keep_prob=1, mini_batch_size=128,
                             num_iterations=5000, learning_rate=0.01, optimizer="adam", beta=0.9, beta1 = 0.9,
                             beta2 = 0.999,  epsilon = 1e-8, print_cost=True)
